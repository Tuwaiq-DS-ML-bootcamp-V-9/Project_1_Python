...
def toggle_status():
    ...
    if 1 <= task_number <= len(grocery_list):
        ...
        toggle = lambda status: "Completed" if status == "Incompleted" else "Incompleted"
        status_dict[selected_item] = toggle(status_dict[selected_item])
        print(Colors.SEA_BLUE + f"\nItem '{selected_item}' updated to '{status_dict[selected_item]}'!" + Colors.ENDC)
